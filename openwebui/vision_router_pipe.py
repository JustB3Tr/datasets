...
class Pipe:
    ...
    def _strip_data_url(url: str) -> Optional[str]:
        ...
        if url.startswith("http"):
            # Refuse remote URLs: fetching them server-side would let the pipe be
            # used as an SSRF primitive. Open WebUI sends attachments as data: URLs.
            return None
    ...
    def pipe(self, body: dict, __event_emitter__=None) -> Generator[str, None, None]:
        ...
        caption_note = (
            "When a message includes '[Image context: ...]' blocks, those are detailed "
            "captions of images the user attached, generated by a vision model. Treat "
            "them as if you saw the image yourself; answer the user's question about "
            "the image using that context. Do not mention the captioning process."
        )
        if out_messages and out_messages[0]["role"] == "system":
            # Preserve the user's system prompt; append the caption instruction.
            out_messages[0]["content"] += "\n\n" + caption_note
        else:
            out_messages.insert(
                0,
                {"role": "system", "content": "You are a helpful assistant. " + caption_note},
            )
